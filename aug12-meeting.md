...
### §3.3 PPE Requirements — **FULLY IMPLEMENTED**
The `personalProtectiveEquipment` field exists on the `Chemical` model (JSON `string[]`). It is populated for all 14 seeded chemicals (realistic arrays: nitrile gloves, splash goggles, lab coats, respirators, face shields, etc.) and the AI auto-fill feature extracts PPE from SDS Section 8.

PPE is rendered in **four** places via a reusable `PpeList` component (`src/components/common/PpeList.tsx`) with inline SVG icons (no remote images, works offline):
- **Catalog cards** (`chemical-card.tsx`) — compact pills
- **Detail view** (`chemical-detail.tsx`) — dedicated "Required PPE" card in the right column + a "PPE Info" popover button in the header
- **Emergency view** (`emergency-view.tsx`) — dedicated "Required PPE" section **AND** an icon-only quick-scan row positioned directly beside the Emergency Contact button (exactly as the meeting proposed)

A normalization layer in `src/lib/ppe.ts` (`normalizePpe()`, `parsePpeText()`, `detectPpeCode()`) parses free-text PPE entries like `"Nitrile gloves (powder-free)"` into structured `{ code, label, note }` objects with deterministic icon assignment (12 canonical PPE codes). This works for legacy seed data, AI-extracted data, and admin-typed text — all without requiring structured input.
